    ...
    public partial class MainWindow : Window
    {
        ...
        private void LoadEmployees()
        {
            try
            {
                ...
                string csvFilePath = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "ExportCSV.csv");

                if (!File.Exists(csvFilePath))
                {
                    MessageBox.Show($"CSV file not found: {csvFilePath}\n\nPlease make sure ExportCSV.csv is in the same folder as the executable.",
                ...
                StatusText.Text = $"Loaded {employees.Count} employees from {Path.GetFileName(csvFilePath)}";
            }
            catch (Exception ex)
            {
